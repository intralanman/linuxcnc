...
int trtkinsSetup(char* compname,
                 char* reqd_coords,
                 char* coordinates,
                 int   comp_id)
{
    #define DISALLOW_DUPLICATES 0
    int njoints = strlen(reqd_coords);
    ...
    for (i=0; i < njoints; i++) {
        ...
        if (   !strchr(coordinates,toupper(reqd_char))
            ...
            rtapi_print_msg(RTAPI_MSG_ERR,
                 ...
                 compname,reqd_coords,coordinates);
            goto error;
        }
    }
    ...
    if((res = hal_pin_float_newf(HAL_IN, &(haldata->x_rot_point), comp_id,
                                "%s.x-rot-point",compname)) < 0)  goto error;
    if((res = hal_pin_float_newf(HAL_IN, &(haldata->y_rot_point), comp_id,
                                "%s.y-rot-point",compname)) < 0)  goto error;
    if((res = hal_pin_float_newf(HAL_IN, &(haldata->z_rot_point), comp_id,
                                "%s.z-rot-point",compname)) < 0)  goto error;
    if((res = hal_pin_float_newf(HAL_IN, &(haldata->x_offset),    comp_id,
                                "%s.x-offset",compname))    < 0)  goto error;
    if((res = hal_pin_float_newf(HAL_IN, &(haldata->y_offset),    comp_id,
                                "%s.y-offset",compname))    < 0)  goto error;
    if((res = hal_pin_float_newf(HAL_IN, &(haldata->z_offset),    comp_id,
                                "%s.z-offset",compname))    < 0)  goto error;
    if((res = hal_pin_float_newf(HAL_IN, &(haldata->tool_offset), comp_id,
                                "%s.tool-offset",compname)) < 0)  goto error;
    return 0;

error:
    rtapi_print_msg(RTAPI_MSG_ERR,"trtkinsSetup() FAIL\n");
    return -1;
} // trtkinsSetup()
